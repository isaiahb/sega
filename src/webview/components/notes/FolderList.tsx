...
import { Star, Circle, Filter, X, ChevronDown } from 'lucide-react';
...
export const FolderList: React.FC<FolderListProps> = ({
  ...
  const activeFilters = [showStarredOnly, showTranscribingOnly].filter(Boolean).length;
    ...
    <div className="h-full flex flex-col bg-zinc-50 dark:bg-zinc-950 border-r border-zinc-200 dark:border-zinc-800">
      ...
      <div className="p-4 border-b border-zinc-200 dark:border-zinc-800 shrink-0">
        <div className="flex items-center justify-between mb-3">
          <h2 className="text-sm font-bold text-zinc-900 dark:text-white uppercase tracking-wider">Days</h2>
                  ...
                  <div className="space-y-4">
                    ...
                    >
                      <div className="flex items-center gap-3">
                        <Star size={18} className={showStarredOnly ? "text-yellow-500" : "text-zinc-400"} />
                        <span className="font-medium text-zinc-900 dark:text-white">Starred Only</span>
                      </div>
                      <div className={clsx(
                    ...
                    <button
                      onClick={() => setShowTranscribingOnly(!showTranscribingOnly)}
                      className={clsx(
                        "w-full flex items-center justify-between p-4 rounded-xl border transition-all",
                    ...
                    >
                      <div className="flex items-center gap-3">
                        <Circle size={18} className={showTranscribingOnly ? "text-emerald-500 fill-emerald-500" : "text-zinc-400"} />
                        <span className="font-medium text-zinc-900 dark:text-white">Active Only</span>
                      </div>
                      <div className={clsx(
                        "w-5 h-5 rounded-full border-2 transition-all",
                        showTranscribingOnly
