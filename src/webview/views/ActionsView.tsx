...
export const ActionsView: React.FC = () => {
  ...
  const loadActions = async () => {
    try {
      ...
      const { data: realActions } = await fetchWithFallback(
        () => api.getActionItems(),
        [],
      ...
      if (realActions && realActions.length > 0) {
        ...
        const transformed = realActions.map((action: ActionItem, idx) => ({
          ...
          noteTitle: action.sourceMeetingId
            ? `Meeting ${action.sourceMeetingId}`
            : "Manual Entry",
          date: new Date(), // Always provide a Date object
        }));
            ...
            <div className="divide-y divide-zinc-100 dark:divide-zinc-800">
              {allActions.length > 0 ? (
                allActions.map((action, i) => (
                  <div
                    key={action.id || i}
                    className="grid grid-cols-12 gap-4 px-6 py-4 items-center hover:bg-zinc-50 dark:hover:bg-zinc-800/50 transition-colors group"
                  >
                    <div className="col-span-5 flex items-start gap-3">
                      <button
                        ...
                        className={clsx(
                          ...
                          action.done
                            ? "text-emerald-500"
                            : "text-zinc-300 dark:text-zinc-600 hover:text-emerald-500 dark:hover:text-emerald-400",
                        )}
                      >
                        {action.done ? (
          ...
          <div className="flex gap-6 h-full overflow-x-auto pb-4 items-start">
            {["To Do", "In Progress", "Done"].map((statusLabel) => {
              ...
              return (
                <div
                  key={statusLabel}
                  className="w-80 flex-shrink-0 flex flex-col max-h-full bg-zinc-100/50 dark:bg-zinc-900/30 rounded-xl p-3 border border-zinc-200 dark:border-zinc-800/50"
                >
                  ...
                  <div className="flex-1 overflow-y-auto space-y-3 pr-1 scrollbar-hide">
                    {columnItems.map((item, idx) => (
                      <div
                        key={idx}
                        className="bg-white dark:bg-zinc-900 p-4 rounded-lg border border-zinc-200 dark:border-zinc-800 shadow-sm hover:shadow-md transition-shadow group cursor-pointer"
                      >
                        ...
                        <p
                          className={clsx(
                            "text-sm font-medium mb-3 line-clamp-2",
                            item.done
